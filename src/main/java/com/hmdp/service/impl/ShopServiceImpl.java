    ...
    public Result queryShopByType(Integer typeId, Integer current, Double x, Double y) {
        ...
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo() // GEOSEARCH key BYLONLAT x y BYRADIUS 10 WITHDISTANCE
                .search(
                        ...
                        RedisGeoCommands.GeoSearchCommandArgs.newGeoSearchArgs().includeDistance()//显示距离
                                .limit(end)//这里只有一个参数，默认从0->end 因此我们需要手动分页
                );
        // 4.解析出id
        if (results == null) {
        ...
        if (list.size() <= from) {
            // 没有下一页了，结束  list.size()包含查询到的全部信息 但总量小于分页开头时，证明当前页没有数据了
            return Result.ok(Collections.emptyList());
        }
        // 4.1.截取 from ~ end的部分
        ...
        list.stream().skip(from).forEach(result -> {
            // 4.2.获取店铺id (这里的getName其实就是获取成员member，我们的member保存的是店铺id)
...
//        // --- 核心修改部分开始 ---
//
//        // 3.1 创建一个以用户当前坐标为圆心，5000米为半径的圆 (Circle)
//        Point center = new Point(x, y);
//        Distance radius = new Distance(5000, RedisGeoCommands.DistanceUnit.METERS);
//        Circle circle = new Circle(center, radius);
//
//        // 3.2 使用 radius 方法传入 Circle 对象，底层会自动调用原生的 GEORADIUS 按坐标查询
...
//
//        list.stream().skip(from).forEach(result -> {
//            // 4.2.获取店铺id (现在查出来的绝对都是纯净的店铺 ID，不需要再判断是不是 "member" 了！)
//            String shopIdStr = result.getContent().getName();
//            ids.add(Long.valueOf(shopIdStr));
//
//            // 4.3.保存距离
